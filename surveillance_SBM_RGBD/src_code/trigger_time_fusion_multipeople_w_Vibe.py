...
dim_x = 640
dim_y = 480
var_name = ',mp2_inp_ViBe'
init_time =0     
end_time = 1399

input_path = "input_multipeople2/" 
...
for jj in range(init_time,end_time+1):
    # print ('Processing time step: ' + str(jj)
    inpfname = input_path + var_name + str(jj) + '.vti'
    # folder_name = outpath + "timestep_" + str(jj)
    # os.mkdir(folder_name)
    bubble_counts[jj-init_time]=segment_feature(inpfname,confidence_th,size_threshold, jj)
    print(jj)
...
output_path = checkorcreatepath("Output/threshold"+str(confidence_th)+"/size"+str(size_threshold)+"/") 
    ...
    if ii == end_time-init_time:
        ...
        if bubble_counts[ii] != bubble_counts[ii+1]:
            ...
            if ii+2 < end_time-init_time+1 and bubble_counts[ii+1] == bubble_counts[ii+2] :
                oname = opath + var_name + str(ii+start_time)+  '.vti'
                ...
                inputfile = input_path + var_name + str(ii+1+init_time) + '.vti'
